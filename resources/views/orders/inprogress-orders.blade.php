<x-app-layout title="In-Progress Orders">
    <x-slot name="header">
                    ...
                    <div class="flex justify-between">
                        <div>
                            <p class="text-xl pb-3 ">
                                <x-heroicon-s-truck class="inline-block h-8"/> In-Progress Orders
                            </p>
                        </div>
                        <div class="pb-3">
                            <x-button class="ml-1 bg-green-600 hover:bg-green-300" onclick="location.href='{{ route('add-order') }}'">
                                {{ __('Add Order')}}
                            </x-button>
                        ...
                        <table class="min-w-full bg-white border-2">
                            ...
                            <form id="boxes" method="GET" action="{{route('remove-multi-order')}}">
                                @foreach($orders as $value)
                                    @if ($value->status == 'Ready to Start' or $value->status == 'In Progress' or $value->status == 'Delayed')
                                            ...
                                            <td>
                                                <div class="flex justify-left items-center">
                                                    ...
                                                    <x-button type="text" class="ml-1 bg-yellow-500 hover:bg-yellow-300"><a
                                                            onclick="location.href='{{ route('edit-order', $value->id) }}'">
                                                            {{ __('Edit') }}
                                                        </a></x-button>
                                                    <x-button type="text" class="ml-1 bg-red-600 hover:bg-red-300"><a
                                                            onclick="location.href='{{ route('remove-order', $value->id) }}'">
                                                            {{ __('Delete') }}
                                                        </a></x-button>
                                                </div>
                                            </td>
